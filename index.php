...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
	...
	$smarty->assign('get_message',     get_message());       //公告
	$arr=get_message();
	
	//var_dump($arr['13']['article']);
	
	$smarty->assign('congshu_menu',     congshu_menu());     //丛书目录
	//var_dump(congshu_menu());
    $smarty->assign('top_goods',       get_top10());           // 销售排行
	//获取点击量较高的商品
	$smarty->assign('maxcount_goods',  get_maxcount_goods());      //获取点击量较高的商品
	...
	//查询商品订单表
	//$sql="select goods_name,goods_id from ".$ecs->table('order_goods')."group by goods_name order by sum(goods_number) desc";
	//$arr=$db->getAll($sql);
...
$aboutus=$db->getOne("select content from ".$ecs->table('article')."where article_id=5");
